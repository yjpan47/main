...
* **Alternative 1 (current choice):** LogIn box handles logic of finding account, MainWindow has a User field.
** Pros: one less layer for simplicity
** Cons: UI best not to handle logic, User Type might be changed, which could be bad for administrator security.
* **Alternative 2:** Logging in is like a command that is handled by logic, after which Main Window is initialized with the UserType and NRIC.
** Pros: Logic operations now handled by Logic class, UserType and NRIC can be final attribute to make sure no one gets the wrong account/ permission for each window.
** Cons: Command from logging in should be different from 'command' class to prevent undos or redos from happening.
// end::login[]
...
|`* * *` |visual user |see all the duties in a user-friendly form |understand the whole schedule better
...
[discrete]
=== Use case 5 (Admin): See all duties planned for current and next month in the calendar
...
[discrete]
=== Use case 6 (Admin): See duties assigned to specific user
...
[discrete]
=== Use case 7 (Admin): Schedule duties
...
[discrete]
=== Use case 8 (Admin): Clear all the database, including duties
...
[discrete]
=== Use case 9 (Admin): Approve swapping of duties
...
[discrete]
=== Use case 10 (General user): See own duties
...
[discrete]
=== Use case 12 (General user): Log into the system
...
[discrete]
=== Use case 13 (General user): Exit the system
