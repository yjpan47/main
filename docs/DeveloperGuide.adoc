...
* `DutyCalendar#rollover()` `planned for v1.3` -- replaces `currentMonth` with `nextMonth`, run duty allocation algorithm on `nextMonth` automatically to allocate duties based on the current state of the month such as blocked dates of each personnel. New empty `nextMonth` object will be instantiated.
* `DutyCalendar#clearMonth(DutyMonth)` `planned for v1.3` -- clear all duties fields or blocked dates in the `DutyMonth` object
* `DutyCalendar#reallocate()` `planned for v1.3` -- rerun the algorithm to randomly reassign the duties again

These operations are to be exposed in the `Model` interface as `Model#rolloverDutyCalendar()`, `Model#clearMonthDutyCalendar(DutyMonth)` and `Model#reallocate()` respectively *for v1.3*.
